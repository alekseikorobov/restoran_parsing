...
def get_best_from_result(df:pd.DataFrame) -> pd.DataFrame:
...
def start(df_zoon_search:pd.DataFrame, control_count) -> pd.DataFrame:
    ...
    for i, row in tqdm(df_process.iterrows(),total=df_process.shape[0]):
        ...
        df_line = get_best_from_result(df_to)
