...
def get_normalization_list(array:list):
...
def get_standart_list(array:list):
...
def get_json_from_search_page_by_company_name(row):
    city_line = dict_city.get_line_by_city_name(row['location_nm_rus'])

    address = row['ya_address']
    for rep in city_line['replaces']:
        address = address.replace(rep,'')
    address = address.replace(', ',' ') 
    query = f"{row['ya_company_name']} {address}"
    search_text = lbyd.get_search_text(query)
    page_name = f'{lbyd.pretty_file_name(search_text)}.json'
    path = common.get_folder(base_folder, city_line['city'],'search_json',None)
    full_name = f'{path}/{page_name}'
    if not os.path.isfile(full_name):

        html_result = lbyd.get_html_by_search_company(city_line['city'], query)

        logging.debug(f'save {full_name=}')
        json_result_list = parse_data.get_items(html_result)
        with open(full_name,'w', encoding='utf') as f:
            json.dump(json_result_list,f, ensure_ascii=False)
    else:
        with open(full_name,'r', encoding='utf') as f:
            json_result_list = json.load(f)

    json_result_list = [
        {replace_key(key):item[key] for key in item } 
        for item in json_result_list
    ]
    return json_result_list

def get_z_query(city_name,ya_point):
...
def parse_search_data(df_process):
    ...
    for i, row in tqdm(df_process.iterrows(), total=df_process.shape[0]):
        ...
        zoom=18
        row['z_query'] = get_z_query(row['location_nm_rus'],row['ya_point'])
        ...
        #скачивание из страницы поиска не стали делать, всё равно результат не очень хороший
        # if row['ignor_load'] == 'ZOON':
        #     logging.debug('add additional info by search company name')
        #     json_list_result_add = get_json_from_search_page_by_company_name(row)
        #     json_result_list.extend(json_list_result_add)
        ...
        if len(json_result_list) == 0:
            ...
            for json_result_item in json_result_list:
                ...
                row['z_address_n'] = common.replace_address_by_city(row['location_nm_rus'], row['z_address'])
...
def start(df_yandex_match:pd.DataFrame)->pd.DataFrame:
