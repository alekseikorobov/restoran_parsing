...
class LoadTripDetails:
    ...
    def update_all(self,row):
        ...
        if not common.isfile(full_name):
            ...
            load_data.get_html_details_and_parse(self.params.cache_data_folder, city, full_url, location_id,
                replace_json=False,
                timeout=self.params.timeout_load_trip_details,
                proxy=self.params.proxy,
                headers=self.params.ta_parser_headers)
        try:
