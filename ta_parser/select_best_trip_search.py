...
class SelectBestTripSearch:
    def __init__(self, params: Params) -> None:
        self.params = params

    def get_location_id_from_url(self, row)->str:
    ...
    def df_first_and_count(self, df_new, group_by = ['col1','col2'], order_by = ['ser','ser1']):
    ...
    def get_except(self, df_all:pd.DataFrame,df_except:pd.DataFrame,cols:list):
    ...
    def start(self, df_trip_search:pd.DataFrame) -> pd.DataFrame:
        logging.debug('start')
        ...
        df_trip_search_not = self.get_except(df_all = df_trip_search,df_except=df_trip_search_to,cols = ['source_id','ya_id'])
        ...
        df_trip_search_not = self.df_first_and_count(df_trip_search_not,group_by=['source_id','ya_id'],order_by=cols)

        logging.debug(f'first {df_trip_search_not.shape=}, {df_trip_search_not["source_id"].nunique()=},{df_trip_search_not["ta_link"].nunique()=}')

        df_result_filter = self.df_first_and_count(df_trip_search_to,group_by=['source_id','ya_id'],order_by=cols)
        ...
        df_result['ta_location_id'] = df_result.apply(self.get_location_id_from_url, axis=1)
