    ...
    def parse_search_data(self, df_process):
        ...
        for i, row in df_process.iterrows():
            ...
            if not common.isfile(full_name_param):
                try:
                    lbyd.save_json_by_search_page(base_folder, city_line['city'], query,timeout=self.params.timeout_load_trip_search,proxy=self.params.proxy,headers=self.params.ta_parser_headers_search)
                    self.curr_error = 0
